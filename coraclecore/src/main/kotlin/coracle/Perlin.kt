...
object Perlin {
    var seed: Double = Random.nextDouble(0.0, 10000.0)

    fun newSeed(){
        seed = Random.nextDouble(0.0, 10000.0)
    }
    ...
    fun noise(x: Float, y: Float): Float{
        return noise(x.toDouble(), y.toDouble(), seed).toFloat()
    }

    fun noise(x: Double, y: Double): Float{
        return noise(x, y, seed).toFloat()
    }
