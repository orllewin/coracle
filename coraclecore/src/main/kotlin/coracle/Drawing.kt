...
abstract class Drawing {
    ...
    fun background() = renderer.background()
    ...
    fun nativeColour(key: String): Int = renderer.nativeColour(key)

    fun loadImage(key: String, onReady: (image: Image?) -> Unit) = renderer.loadImage(key, onReady)
    fun image(image: Image) = renderer.image(image, 0, 0)
    fun image(image: Image, x: Int, y: Int) = renderer.image(image, x, y)
    fun image(image: Image, x: Int, y: Int, width: Int, height: Int) = renderer.image(image, x, y, width, height)

    fun loadPixels() = renderer.loadPixels()
    fun pixel(x: Int, y: Int): Int? = renderer.pixel(x, y)
    ...
    fun line(pA: Point, pB: Point) = renderer.line(pA.x.toInt() + xTranslation, pA.y.toInt() + yTranslation, pB.x.toInt() + xTranslation, pB.y.toInt() + yTranslation)

    fun triangle(triangle: Triangle) = renderer.triangle(triangle.a.x.toInt(), triangle.a.y.toInt(), triangle.b.x.toInt(), triangle.b.y.toInt(), triangle.c.x.toInt(), triangle.c.y.toInt())
    ...
    fun circle(p: Point, r: Int) = renderer.circle(p.x.toInt() + xTranslation, p.y.toInt() + yTranslation, r)

    fun point(p: Point) = renderer.point(p.x.toInt() + xTranslation, p.y.toInt() + yTranslation)
    fun point(x: Int, y: Int) = renderer.point(x + xTranslation, y + yTranslation)
    fun point(x: Double, y: Double) = renderer.point(x.toInt() + xTranslation, y.toInt() + yTranslation)
